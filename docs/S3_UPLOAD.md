...
| Стиль адресации | Выбор между Path-style и Virtual-host style. | `Path-style` (для MinIO/NPM) |
...
#### Выбор стиля адресации (Addressing Style)

*   **Auto (по умолчанию)**: Плагин автоматически выбирает стиль. Если эндпоинт не является стандартным для AWS (`amazonaws.com`), содержит порт или IP-адрес, будет использован **Path-style**.
*   **Path-style**: Ссылки формируются как `https://domain.com/bucket/key`. **Обязательно** для большинства self-hosted решений, таких как MinIO за Nginx Proxy Manager (NPM).
*   **Virtual-host style**: Ссылки формируются как `https://bucket.domain.com/key`. Используется в AWS S3 и требует поддержки wildcard-поддоменов в DNS и прокси-сервере.

> **Важно для пользователей Nginx Proxy Manager**: Если вы используете NPM для доступа к MinIO (например, `s3.m3zold-lab.tech`), установите **Path-style**, чтобы избежать ошибок `network-error` из-за отсутствия поддомена для бакета.
